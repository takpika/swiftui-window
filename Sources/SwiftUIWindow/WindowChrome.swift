            ...
            .onChange(of: windowMode) {value in
                if (value != nil && value != WindowPosMode.normal) {
                    if let value = value {
                        ...
                        destPos = CGPoint(x: destPos.x + destSize.width * 0.05, y: destPos.y + destSize.height * 0.05)
                        destSize = CGSize(width: destSize.width * 0.9, height: destSize.height * 0.9)
                        moveWindow(destPos: destPos, destSize: destSize)
                    }
                } else {
...
public enum WindowPosMode: String {
    case normal = "normal"
    case center = "center"
...
public struct WindowConfig {
    ...
    public init(
        ...
        resizable: Bool = true,
        showLabel: Bool = true,
        showWindowBar: Bool = true,
        startPos: WindowPosMode = WindowPosMode.normal
    ) {
        self.title = title
        self.size = size
        ...
        self.showWindowBar = showWindowBar
